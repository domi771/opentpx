...
describe TPX_2_2::Exchange do
  subject {
    TPX_2_2::Exchange.new(
      provider_s: 'provider',
  ...
  let(:collection) {
    TPX_2_2::Collection.new([ collection_element_1, collection_element_2 ])
  }
  let(:network) {
    TPX_2_2::Network.new(
      asn_number_ui: 1,
  ...
  describe '#<<' do
    ...
    it 'should not add duplicate items to a collection' do
      ...
      expect{
        ...
        'Duplicate input object id MarketSeg1 provided to TPX_2_2::Collection!'
      )
    end
  end
  ...
  context "(import methods)" do
    context "when supplied with valid input" do
      let(:valid_tpx_filelist) {Dir.glob(File.expand_path(File.join(File.dirname(File.expand_path(__FILE__)), '..', '..', '..', '..', 'examples', 'tpx', 'valid', '*')))}
      let(:valid_tpx_file) {File.expand_path(File.join(File.dirname(File.expand_path(__FILE__)), '..','..', '..', '..','examples','tpx','valid','pass.json'))}
      let(:valid_input_hash) {Oj.load_file(valid_tpx_file)}

      describe ".import" do
        it "should not raise exceptions" do
          expect {TPX_2_2::Exchange.import(valid_input_hash)}.not_to raise_error
        end

        it "should create valid Exchange" do
          exchange = TPX_2_2::Exchange.import(valid_input_hash)
      ...
      describe ".import_file" do
        it "should not raise errors" do
          valid_tpx_filelist.each do |f|
            expect {
              begin
                TPX_2_2::Exchange.import_file(f)
              rescue => e
                raise e unless e.is_a?(TPX_2_2::ValidationWarning)
              end
            }.not_to raise_error
          end
        end
        ...
        it "should create valid Exchange" do
          exchange = TPX_2_2::Exchange.import_file(valid_tpx_file)
          expect(exchange["element_observable_c_array"].size).to eq valid_input_hash["element_observable_c_array"].size
          expect(exchange["observable_dictionary_c_array"]).to eq valid_input_hash["observable_dictionary_c_array"]
        end
      end
    ...
    context "when supplied valid input but with undefined observable dictionary entry" do
      let(:valid_tpx_filelist) {Dir.glob(File.expand_path(File.join(File.dirname(File.expand_path(__FILE__)), '..','..','..','..','examples','tpx','invalid','undefined_observable.json')))}
      describe ".import_file" do
        it "should raise exceptions" do
          valid_tpx_filelist.each do |f|
            expect {TPX_2_2::Exchange.import_file(f)}.to raise_error TPX_2_2::ValidationWarning
          end
        end
      end
    end
    ...
    context "when supplied with invalid input" do
      let(:invalid_tpx_filelist) do 
        pathes = Dir.glob(File.expand_path(File.join(File.dirname(File.expand_path(__FILE__)), '..', '..', '..', '..', 'examples', 'tpx', 'invalid', 'incorrect*')))
        pathes.concat(Dir.glob(File.expand_path(File.join(File.dirname(File.expand_path(__FILE__)), '..', '..', '..', '..', 'examples', 'tpx', 'invalid', 'invalid*'))))
        pathes.concat(Dir.glob(File.expand_path(File.join(File.dirname(File.expand_path(__FILE__)), '..', '..', '..', '..', 'examples', 'tpx', 'invalid', 'mixed_format.json'))))
      end
      describe ".import_file" do
        it "should raise exceptions" do
          invalid_tpx_filelist.each do |f|
            expect {TPX_2_2::Exchange.import_file(f)}.to raise_error TPX_2_2::ValidationError
          end
        end
      end
    end
  ...
  describe '#to_manifest with manifest files' do
    ...
    it 'should return correct hash with observable_element_file_manifest' do
      ...
      expect(subject.send(:to_manifest, 'spec/data/test.json')).to eq({
        ...
        "distribution_time_t" => distribution_time,
        "last_updated_t" => distribution_time,
        "observable_element_file_manifest" => ["data_1.json", "data_2.json"],
      })
    end
    ...
    it 'should return correct hash with collection_file_manifest' do
      ...
      expect(subject.send(:to_manifest, 'spec/data/test.json')).to eq({
        ...
        "distribution_time_t" => distribution_time,
        "last_updated_t" => distribution_time,
        "collection_file_manifest" => ["collection_1.json", "collection_2.json"]
      })
    end
