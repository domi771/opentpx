...
describe TPX_2_2::Validator do
  ...
  describe '##validate_schema!' do
    it 'should not raise error when supplied with valid TPX' do
      expect{TPX_2_2::Validator.send(:validate_schema!, schema )}.to_not raise_error
    end
    it 'should raise error when supplied with invalid TPX' do
      s = schema.dup.delete('provider_s')
      expect{TPX_2_2::Validator.send(:validate_schema!, s )}.to raise_error(TPX_2_2::ValidationError)
    end
  end

  describe '##validate_collections!' do
    it 'should validate collections' do
      expect(TPX_2_2::Validator.send(:validate_collections!, [collection])).to eq([collection])
    end
    it 'should raise error when supplied with invalid collection' do
      collection.delete(:name_id_s)
      expect{TPX_2_2::Validator.send(:validate_collections!, [collection])}.to raise_error(TPX_2_2::ValidationError, /The property \'#\/\' did not contain a required property of \'name_id_s\' in schema /)
    end
  end

  describe '##validate_element_observables!' do
    it 'should validate element observables' do
      expect(TPX_2_2::Validator.send(:validate_element_observables!, [element_observable])).to eq([element_observable])
    end
    it 'should raise error when supplied with invalid element observable' do
      element_observable.delete(:threat_observable_c_map)
      expect{TPX_2_2::Validator.send(:validate_element_observables!, [element_observable])}.to raise_error(TPX_2_2::ValidationError, /The property \'#\/\' did not contain a required property of \'threat_observable_c_map\' in schema /)
    end
  end

  describe '##validate_networks!' do
    it 'should validate networks' do
      expect(TPX_2_2::Validator.send(:validate_networks!, [network])).to eq([network])
    end
    it 'should raise error when supplied with invalid networks' do
      network.delete(:asn_number_ui)
      expect{TPX_2_2::Validator.send(:validate_networks!, [network])}.to raise_error(TPX_2_2::ValidationError, /The property \'#\/\' did not contain a required property of \'asn_number_ui\' in schema /)
    end
  end

  describe '##validate_observable_dictionary!' do
    it 'should not raise error when supplied with valid observable dictionary' do
      expect{TPX_2_2::Validator.send(:validate_observable_dictionary!, [observable_dictionary], [element_observable])}.to_not raise_error
    end
    it 'should raise error when supplied with invalid observable dictionary' do
      ...
      expect{TPX_2_2::Validator.send(:validate_observable_dictionary!, [observable_dictionary], [element_observable])}.to raise_error(TPX_2_2::ValidationError, /The property \'#\/\' did not contain a required property of \'classification_c_array\' in schema/)
    end
  end
  ...
  context 'when supplied with valid input' do
    describe '##validate_json!' do
      it 'should not raise error' do
        expect{TPX_2_2::Validator.validate_json!(schema.to_json)}.to_not raise_error
      end
    end

    describe '##validate!' do
      it 'should not raise error' do
        expect{TPX_2_2::Validator.validate!(schema)}.to_not raise_error
      end
    end

    it 'should not raise error' do
      file = Tempfile.new('TPXValidator')
      file.write(schema.to_json)
      file.rewind
      expect{TPX_2_2::Validator.validate_file!(file.path)}.to_not raise_error
    end
    ...
    context 'when observable has no corresponding entry in the observable dictionary' do
      describe '##validate_file!' do
        ...
        it 'should raise exceptions' do
          expect {TPX_2_2::Validator.validate_file!(file_to_validate)}.to raise_error TPX_2_2::ValidationWarning
        end
      end
    end
    ...
    describe '##validate_file!' do
      ...
      it 'should raise exceptions' do
        invalid_validator_filelist.each do |f|
          expect {TPX_2_2::Validator.validate_file!(f)}.to raise_error TPX_2_2::ValidationError
        end
      end
    end
  end

  context 'when supplied with valid TPX manifest as input' do
    describe '##validate_file!' do
      let(:manifest) {Dir.glob(File.expand_path(File.join(File.dirname(File.expand_path(__FILE__)), '..', 'validator_data', 'manifest*')))}
      it 'should not raise validation error' do
        manifest.each do |f|
          expect {TPX_2_2::Validator.validate_file!(f)}.to raise_error TPX_2_2::ValidationWarning
        end
      end
    end
  end
