...
describe TPX_2_2::ElementObservableList do
  subject { TPX_2_2::ElementObservableList.new(elements) }

  let(:elements) { [element_observable_1, element_observable_2] }
  let(:element_observable_1) { TPX_2_2::ElementObservable.new(element_observable_hash_1) }
  let(:element_observable_2) { TPX_2_2::ElementObservable.new(element_observable_hash_2) }
  let(:element_observable_3) { TPX_2_2::ElementObservable.new(element_observable_hash_3) }
  let(:element_observable_4) { TPX_2_2::ElementObservable.new(element_observable_hash_4) }
  ...
  describe '#initialize' do
    ...
    it 'should warn on duplicate input elements' do
      expect{
        TPX_2_2::ElementObservableList.new([element_observable_1, element_observable_1])
      }.to raise_error(
    ...
    it 'should ensure all items are the proper type' do
      expect{
        TPX_2_2::ElementObservableList.new({foo: 'bar'})
      }.to raise_error(
  ...
  describe '#<<' do
    it 'should ensure all items are the proper type' do
      expect{subject << {foo: 'bar'}}.to raise_error(TPX_2_2::ValidationError)
    end
