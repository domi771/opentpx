shared_context 'a homogeneous list of data models' do |default_creation_opts={}|
  ...
  describe '#initialize' do
    ...
    it 'should raise a validation error when the wrong object type is provided' do
      expect{ subject.class.new([InvalidElementClassFoo]) }.to raise_error(TPX_2_1::ValidationError)
    end
  end
  ...
  describe '#<<' do
    ...
    it 'should raise a validation error when the wrong object type is provided' do
      expect{ subject << InvalidElementClassFoo }.to raise_error(TPX_2_1::ValidationError)
    end

    it 'should raise a validation error when an object with a duplicate id is inserted' do
      expect{ subject << subject.first }.to raise_error(TPX_2_1::DuplicateElementInsertError)
    end
  end
