shared_context 'a merging homogeneous list of data models' do |default_creation_opts={}|
  ...
  describe '#initialize' do
    ...
    it 'should raise a validation error when the wrong object type is provided' do
      expect{ subject.class.new([InvalidElementClassFoo]) }.to raise_error(TPX_2_1::ValidationError, "Supplied input object InvalidElementClassFoo at position 0 not of required type #{subject.class.type} in #{subject.class}!")
    end
  end
  ...
  describe '#<<' do
    ...
    it 'should raise a validation error when the wrong object type is provided' do
      expect{ subject << InvalidElementClassFoo }.to raise_error(TPX_2_1::ValidationError, "Element provided to #{subject.class}#<< must be #{subject.class.type} with key `#{subject.class.child_id}`.")
    end
