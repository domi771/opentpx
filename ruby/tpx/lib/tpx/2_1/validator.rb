...
module TPX_2_1
  class Validator
    ...
    class << self
      ...
      # @example
      #   TPX_2_1::Validator.validate_file!('folder_name/file_name.json')
      def validate_file!(filepath)
        if File.exists? filepath
          ...
          raise ValidationError, "File [#{filepath}] does not exist"
        end
      end
      ...
      def validate!(input_hash)
        ...
        if (TPX_KEYS & input_hash.keys).length > 0 &&  tpx_manifest?(input_hash)
          raise TPX_2_1::ValidationError, "TPX file must either be in single-file or manifest format."
        end
        ...
        if (input_hash['observable_dictionary_c_array'].nil? || input_hash['observable_dictionary_c_array'].empty?) and input_hash['element_observable_c_array'].present?
          raise TPX_2_1::ValidationWarning, "observable dictionary is not defined"
        end
        unless @@undefined_observables.empty?
          raise TPX_2_1::ValidationWarning, "observables #{@@undefined_observables.inspect} are not defined in the observable dictionary"
        end
      end
      ...
      def validate_schema!(input_hash)
        errors = JSON::Validator.fully_validate(TPX_2_1::SCHEMA, input_hash)
      ...
      def validate_observable_dictionary!(observable_dictionaries, element_observable_list)
        ...
        observable_dictionaries.each do |observable_dictionary|
          ...
          raise TPX_2_1::ValidationError, "#{errors.join(' ')}" unless errors.empty?
